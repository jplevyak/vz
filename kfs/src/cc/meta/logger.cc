...
// default values
string KFS::meta::LOGDIR("./kfslog");
string KFS::meta::LASTLOG(LOGDIR + "/last");

Logger KFS::meta::oplog(LOGDIR);

static KFS::meta::LogRotater logRotater;
...
void
KFS::meta::logger_setup_paths(const string &logdir)
{
	if (logdir != "") {
		LOGDIR = logdir;
		LASTLOG = LOGDIR + "/last";
